...
def register():
    if request.method == "POST":
        ...
        cursor.execute("SELECT * FROM mailer_users WHERE email = %s", (email,))
        ...
        if existing_user:
            ...
            cursor.execute(
                "INSERT INTO mailer_users (email, full_name, phone, password) VALUES (%s, %s, %s, %s)",
...
def verify_email(token):
    ...
    try:
        ...
        cursor = db.cursor()
        cursor.execute(
            "UPDATE mailer_users SET is_verified = true WHERE email = %s", (email,)
        )
        db.commit()
...
def login():
    if request.method == "POST":
        ...
        password = request.form["password"]
        cursor.execute(
            "SELECT * FROM mailer_users WHERE email = %s AND password = %s",
            (email, password),
        )
        ...
        if user and password == user[3]:
            if user[4] == 0:
                flash(
                    "You have not verified your Email yet. Kindly verify then login again."
                )
            else:
...
def reset_password(token):
    ...
    if request.method == "POST":
        ...
        if new_password == confirm_password:
            ...
            cursor = db.cursor()
            cursor.execute(
                "UPDATE mailer_users SET password = %s WHERE email = %s",
                (new_password, email),
            )
            db.commit()
